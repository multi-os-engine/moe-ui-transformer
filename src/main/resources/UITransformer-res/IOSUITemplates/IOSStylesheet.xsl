...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<!-- Start inclides -->
	<xsl:include href="LinearLayoutTemplate.xsl" />
	<xsl:include href="ButtonTemplate.xsl" />
	<!-- End inclides -->
		...
		<xsl:element name="document">
			...
			<xsl:element name="scenes">
				<xsl:element name="scene">
					<xsl:attribute name="sceneID"><xsl:value-of select="osxui:CreateId()"/></xsl:attribute>
					<xsl:element name="objects">
						...
						<xsl:element name="placeholder">
							<xsl:attribute name="placeholderIdentifier">IBFilesResponder</xsl:attribute>
						...
						<xsl:element name="viewController">
							...
							<xsl:attribute name="customModuleProvider"></xsl:attribute>
