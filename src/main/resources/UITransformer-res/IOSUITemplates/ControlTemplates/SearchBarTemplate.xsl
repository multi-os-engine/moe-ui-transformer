...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:strip-space elements="searchBar" />
    <xsl:output method="xml" indent="yes"/>

    <xsl:template match="com.android.sdklib.widgets.iOSSearchBar"
        ...
        <xsl:element name="searchBar">

            <xsl:variable name="styleId" select="@style" />
            <xsl:variable name="searchBarStyles" select="document($g_stylePath)//style[@name = osxui:StyleIdConverter($styleId)]/item" />
            ...
            <xsl:attribute name="contentMode">redraw</xsl:attribute>
            ...
            <xsl:element name="textInputTraits">
                <xsl:attribute name="key">textInputTraits</xsl:attribute>
            </xsl:element>
