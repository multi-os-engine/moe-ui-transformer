...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:strip-space elements="tabBar" />
    <xsl:output method="xml" indent="yes"/>

    <xsl:template match="com.android.sdklib.widgets.iOSTabBar"
        ...
        <xsl:element name="tabBar">
            ...
            <xsl:element name="color">
                ...
                <xsl:attribute name="white">0.0</xsl:attribute>
                <xsl:attribute name="alpha">0.0</xsl:attribute>
                <xsl:attribute name="colorSpace">calibratedWhite</xsl:attribute>
            </xsl:element>

            <xsl:element name="items">
                <xsl:apply-templates select="com.android.sdklib.widgets.iOSTabBarItem" />
            </xsl:element>
            ...
            <xsl:apply-templates select="com.android.sdklib.widgets.iOSTabBar" />
        </xsl:element>
    </xsl:template>

    <xsl:template match="com.android.sdklib.widgets.iOSTabBarItem"
                  xmlns:android="http://schemas.android.com/apk/res/android"
                  xmlns:esi="http://www.edge-delivery.org/esi/1.0">

        <xsl:element name="tabBarItem">
