...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:strip-space elements="seekbar" />
	<xsl:output method="xml" indent="yes"/>
	
	<xsl:template match="com.android.sdklib.widgets.iOSSlider"
		xmlns:android="http://schemas.android.com/apk/res/android"
		xmlns:xrt="http://schemas.android.com/apk/res/android"
		xmlns:esi="http://www.edge-delivery.org/esi/1.0">
		
		<xsl:element name="slider">
			
			<xsl:variable name="styleId" select="@style" />
			<xsl:variable name="sliderStyles" select="document($g_stylePath)//style[@name = osxui:StyleIdConverter($styleId)]/item" />
			...
			<xsl:attribute name="minValue">0</xsl:attribute>
			<xsl:attribute name="maxValue">1</xsl:attribute>
			<xsl:attribute name="translatesAutoresizingMaskIntoConstraints">NO</xsl:attribute>

			<xsl:element name="color">
				<xsl:attribute name="key">minimumTrackTintColor</xsl:attribute>
			...
			<xsl:element name="color">
				<xsl:attribute name="key">maximumTrackTintColor</xsl:attribute>
				<xsl:attribute name="colorSpace">calibratedRGB</xsl:attribute>
