...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="com.android.sdklib.widgets.iOSViewController"
        xmlns:android="http://schemas.android.com/apk/res/android"
        xmlns:esi="http://www.edge-delivery.org/esi/1.0"
        xmlns:xrt="http://schemas.android.com/apk/res/android">

        <xsl:element name="viewController">
            <xsl:attribute name="id"><xsl:value-of select="osxui:CreateId()"/></xsl:attribute>
            <xsl:attribute name="customClass">ViewController</xsl:attribute>
            ...
            <xsl:element name="layoutGuides">
                <xsl:element name="viewControllerLayoutGuide">
                    <xsl:attribute name="type">top</xsl:attribute>
                    <xsl:attribute name="id"><xsl:value-of select="osxui:CreateId()" /></xsl:attribute>
                </xsl:element>
                <xsl:element name="viewControllerLayoutGuide">
                    <xsl:attribute name="type">bottom</xsl:attribute>
                    <xsl:attribute name="id"><xsl:value-of select="osxui:CreateId()" /></xsl:attribute>
                </xsl:element>
            </xsl:element>
            ...
            <xsl:element name="view">
                ...
                <xsl:element name="subviews">
                   <xsl:apply-templates select="*[name() != 'com.android.sdklib.widgets.base_elements.iOSNavigationBarContainer']" />
                </xsl:element>

            </xsl:element>

            <xsl:apply-templates select="com.android.sdklib.widgets.base_elements.iOSNavigationBarContainer"/>
        </xsl:element>
